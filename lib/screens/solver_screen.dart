...
class _SolverScreenState extends State<SolverScreen> {
  ...
  String? errorMessage;
  dynamic result; // Holds the specific result object based on method
  ...
  void _loadSample() {
    setState(() {
      n = 3;
      _buildControllers();
      // Sample diagonally dominant matrix for stability
      ...
      // Expected solution: x1=1, x2=1, x3=1
      errorMessage = null;
      solution = null;
      result = null;
    });
      ...
      switch (widget.method) {
        ...
        case SolverMethod.jacobi:
          final res = IterativeMethods.jacobi(A, b, 
            ...
            maxIterations: int.parse(maxIterController.text));
          solution = res.solution;
          result = res;
          break;
      ...
      return _buildGenericStepView(res.steps.length, (idx) {
        ...
        return Column(
          children: [
            ...
            _buildMatrixWithVector(step.matrix, step.vector),
          ],
        );
      });
    }
    ...
    if (result is InversionResult) {
      ...
      return _buildGenericStepView(res.steps.length, (idx) {
        ...
        return Column(
          children: [
            Text(step.description, style: const TextStyle(fontWeight: FontWeight.bold)),
            _buildMatrixDisplay("", step.matrix),
          ],
        );
      });
    }
    ...
    if (result is IterativeResult) {
      ...
      return Column(
        children: [
          ...
          SizedBox(
            height: 200,
            child: LineChart(_buildChartData(res.steps)),
          ),
      ...
      children: [
        Row(
          ...
          children: [
            ...
            IconButton(
              onPressed: currentStep < count - 1 ? () => setState(() => currentStep++) : null,
  ...
  LineChartData _buildChartData(List<IterationStep> steps) {
    return LineChartData(
      lineBarsData: [
        LineChartBarData(
          spots: steps.map((s) => FlSpot(s.iteration.toDouble(), s.error)).toList(),
          isCurved: true,
          color: Colors.indigo,
      ...
      child: Column(
        children: List.generate(matrix.length, (i) {
          return Row(
            ...
            children: [
              Text("[ ${matrix[i].map((c) => c.toString()).join('  ')} ]"),
  ...
  Widget _buildMatrixDisplay(String label, List<List<ComplexNumber>> matrix) {
    return Column(
      children: [
        ...
        ...matrix.map((row) => Text("[ ${row.map((c) => c.toString()).join('  ')} ]")),
      ],
    );
  }
  ...
  Widget _buildValidationView() {
    ...
    double totalError = 0;
    for (int i = 0; i < n; i++) {
      totalError += (ax[i] - b[i]).magnitude;
    }

    return Card(
      color: totalError < 1e-6 ? Colors.green.shade50 : Colors.orange.shade50,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ...List.generate(n, (i) => Text("Row ${i + 1}: Ax = ${ax[i]} (b = ${b[i]})")),
            const Divider(),
            Text(
              "Total Absolute Error: ${totalError.toStringAsExponential(4)}",
              style: TextStyle(
                fontWeight: FontWeight.bold,
                color: totalError < 1e-6 ? Colors.green : Colors.red,
              ),
            ),
            if (totalError < 1e-6)
              const Text("Status: VALID", style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold))
            else
              const Text("Status: HIGH ERROR", style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
          ],
        ),
      ),
    );
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(builder: (c) => TutorialScreen(topic: title)),
            ),
        ...
        child: Column(
          children: [
            ...
            if (widget.method == SolverMethod.jacobi || widget.method == SolverMethod.gaussSeidel)
              Row(
                children: [
                  Expanded(child: TextField(controller: toleranceController, decoration: const InputDecoration(labelText: 'Tolerance'))),
                  const SizedBox(width: 10),
                  Expanded(child: TextField(controller: maxIterController, decoration: const InputDecoration(labelText: 'Max Iter'))),
                ],
              ),
            ...
            if (solution != null) ...[
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text("Solution x:", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                  IconButton(
                    ...
                    onPressed: () async {
                      ...
                      await PdfExport.generateReport(
                        methodName: title,
                        A: A,
                        b: b,
                        solution: solution!,
              ...
              ...List.generate(solution!.length, (i) => Text("x${i+1} = ${solution![i]}")),
            ],
          ],
        ),
      ),
        ...
        return Row(
          ...
          children: [
            ...List.generate(n, (j) => Padding(
              ...
              child: SizedBox(width: 60, child: TextField(controller: matrixControllers[i][j], decoration: const InputDecoration(border: OutlineInputBorder(), isDense: true))),
            )),
            const SizedBox(width: 10),
            Padding(
              padding: const EdgeInsets.all(2),
              child: SizedBox(width: 60, child: TextField(controller: vectorControllers[i], decoration: const InputDecoration(border: OutlineInputBorder(), isDense: true, labelText: 'b'))),
            ),
          ],
        );
      }),
